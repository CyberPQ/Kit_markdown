...
function CodeBlock(block)
    ...
    -- unchanged.
	if not block.classes[1] then
      return nil
    end
	local class_name = block.classes[1]:gsub('{', ''):gsub('%.', '')
	--local class_name = block.classes[1]
    if not class_name then
      return nil
    end
    local img_converter = converters[class_name]
	io.stderr:write(string.format("block.classes[1] = '%s'     class_name = '%s'\n", block.classes[1], class_name))
